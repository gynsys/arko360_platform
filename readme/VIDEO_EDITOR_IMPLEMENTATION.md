...
- **Barra Contextual Vertical, Opacidad en Formas, Selector de Color y Conmutador de Fondo (`ContextualBar.jsx`, `SlideCanvas.jsx` & `index.jsx`):**
  - **Barra Contextual Vertical:** Se reubicó la barra flotante de edición a la izquierda de la diapositiva (`right-full mr-4 top-1/2 -translate-y-1/2`) en orientación vertical (`flex-col`), creando un espacio de trabajo simétrico.
  - **Opacidad en Formas:** Se extendió el slider de opacidad (`<FiEye />`) a formas geométricas, íconos y textos, respetando `el.opacity` en `SlideCanvas.jsx`.
  - **Corrección de Selector de Color Chromium (0,0):** Se utilizó un ref `colorInputRef` dinámico posicionado en viewport (`fixed z-[9999]`) activado con `.showPicker()`, eliminando el fallo donde Chrome abría la paleta del sistema en la esquina `(0,0)`. Se adaptó `SlideCanvas.jsx` para evaluar `IconComp` en todas las figuras aplicando `el.color` inmediatamente.
  - **Selector "Fondo Perfil: ON/OFF":** Se incluyó un botón flotante superior para encender u ocultar la imagen de fondo configurada en "Mi Perfil".
  - **Corrección del Botón Guardar:** Se vinculó `activeProjectId` en `useSlideDesigner.js` e `index.jsx` para que **GUARDAR** actualice el proyecto abierto directamente sin abrir popups de renombrado (exclusivo de **GUARDAR COMO...**).
