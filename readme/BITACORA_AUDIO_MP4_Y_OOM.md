...
### O. Solución Definitiva al Bug de Posicionamiento Chromium (0,0) del Selector de Color y Aplicación en Formas
- **Eliminación del Salto a (0,0):** Se solucionó el fallo de Chromium en Windows donde la ventana nativa de selección de color se abría en `(0,0)` cuando el input estaba dentro de un contenedor con `backdrop-blur-xl`. En [ContextualBar.jsx](file:///c:/Users/pablo/Documents/arko360_platform/admin/src/modules/biblioarko/pages/social-generator/components/ContextualBar.jsx#L25), se implementó un `colorInputRef` fijo (`className="fixed z-[9999]"`) fuera del contenedor con filtro. La función `handleOpenColorPicker` obtiene el `getBoundingClientRect()` del botón e invoca `showPicker()` en la posición exacta en pantalla.
...
- **Solución:** En [SlideCanvas.jsx](file:///c:/Users/pablo/Documents/arko360_platform/admin/src/modules/biblioarko/pages/social-generator/components/SlideCanvas.jsx#L595), se limitó el `fontSize` únicamente a los íconos (`el.type === 'icon'`) y se inyectó `preserveAspectRatio={el.type === 'shape' ? 'none' : undefined}` a la forma. Ahora, al estirar una forma de lado a lado o arriba/abajo, la figura gráfica escala de manera flexible cubriendo el 100% del ancho y alto del contenedor azul.
