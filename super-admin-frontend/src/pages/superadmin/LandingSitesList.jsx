...
export default function LandingSitesList() {
  ...
  const deleteSite = async (site) => {
    if (!window.confirm(`¿Estás seguro de que deseas eliminar el sitio clonado para ${site.nombre_cliente}?`)) return;
    try {
      const token = localStorage.getItem('arko_admin_token');
      const response = await fetch(`${API_URL}/arko/landing_sites/${site.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
